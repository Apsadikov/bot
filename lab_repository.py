...
def is_user_has_unfinished_labs(user_id):
    sql = "SELECT * FROM progress WHERE user_id = (%s) AND is_finished = false"
    value = [user_id]
    cursor = database.cursor()
    cursor.execute(sql, value)
    result = cursor.fetchall()
...
def is_user_finished_lab(user_id, lab_id):
    sql = "SELECT * FROM progress WHERE user_id = (%s) AND is_finished = true AND lab_id = (%s)"
...
def create_lab():
    sql = "INSERT INTO lab (answer_spreadsheet_link) VALUES (null) RETURNING id"
    cursor = database.cursor()
    cursor.execute(sql)
